...
class ExamSchedulingSystem {
        ...
        Object.entries(coursesBySemester).forEach(([semester, courses]) => {
            courses.forEach((course, courseIndex) => {
                for (let i = 0; i < course.students; i++) {
                    ...
                    const year = semester === "SEMESTER 2" ? "2024" : semester === "SEMESTER 4" ? "2022" : semester === "SEMESTER 6" ? "2021" : "2020";
                    const rollNumber = `${year}BTech${String(courseIndex * 1000 + i + 1).padStart(3, '0')}`;
    ...
    generateRoomSeats(room) {
        ...
        for (let row = 0; row < room.rows; row++) {
            const colsInRow = room.cols_per_row[row] || 10;
    ...
    findOptimalSeat(seats, courseCode, usedSeats) {
        // Anti-cheating algorithm: avoid placing same course students adjacent
    ...
    generateRoomHTML(assignment) {
        ...
        for (let row = 0; row < room.rows; row++) {
            ...
            html += `<div class="room-row-label">${rowLabels[row]}</div>`;
            
            const colsInRow = room.cols_per_row[row] || 10;
            for (let col = 0; col < colsInRow; col++) {
                const seat = seats.find(s => s.row === row && s.col === col);
                const seatClass = seat && seat.occupied ? 'seat occupied' : 'seat';
                const bgColor = seat && seat.occupied ? this.courseColorMap.get(seat.courseCode) : '';
                
                html += `
                    <div class="${seatClass}" 
                         data-seat-id="${seat ? seat.id : ''}"
                         data-student='${seat && seat.student ? JSON.stringify(seat.student) : ''}'
                         data-course="${seat ? seat.courseCode : ''}"
